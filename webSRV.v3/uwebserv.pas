...
uses
  ...
  Synautil, strutils, AppEvnts, Menus, inifiles, das_const,ZAbstractRODataset, ZAbstractDataset,
    ZDataset, ZAbstractConnection, ZConnection, ZCompatibility, zlibexapi,ZLIBEX;
...
//  writeTimeLog('generate begin');
  ...
  for i := 0 to inlist.Count - 1 do
    if i<inlist.Count - 1 then  str1 := str1 + inlist[i] + ', ';
  str1 := str1 + ']';
...
//  jn,jval :tlkjsonnumber;
  ...
  begin
    if length(varval)=0 then varval :='" "';
    if pos('"',varval)=0 then varval :='"'+varval+'"';
    resrecord := resrecord + '"' + varname + '" : ' + varval + ', ';
  end;


procedure writefiles();
procedure writeWebFiles(filename, filedata:string);
var
 ff:tfilestream;
begin
...
begin
      ...
      Backup_history := 'd:\www\mob\JSON_last_data.js';
      if fileexists(Backup_history) then
        deletefile(Backup_history);
      ff := tfilestream.Create(Backup_history, fmcreate or fmOpenWrite or fmsharedenynone);
      ff.Write(str1[1], length(str1));
      ff.Free;
      writeWebFiles('d:\www\mob\JSON_last_data.js', str1);
      writeWebFiles('d:\www\JSON_last_data.js', str1);
      writeWebFiles('d:\www\kguv2\JSON_last_data.js', str1);

      writeWebFiles('d:\www\last_data30.js', jsonstr);
      writeWebFiles('d:\www\mob\last_data30.js', jsonstr);
      writeWebFiles('d:\www\kguv2\last_data30.js', jsonstr);
...
//      memo2.Lines[memo2.Lines.Count - 1] := memo2.Lines[memo2.Lines.Count - 1] + ' fin';
  ...
  if ((now - prevUpdate) * 24 * 3600 < 10) then
  begin
    timer1.Enabled := true;
    exit;
  end;
...
//    r1 := strToFloat(r[11]) - strtofloat(r[12]);
    AddVarToRecord('k1dt5t6', ' ');
//    r1 := strToFloat(r[9]) - strtofloat(r[10]);
    AddVarToRecord('k1dt3t4', ' ');
...
//    writetimelog('## Update History = ' + resrecord);
    ...
    if (linenum mod 5) = 0 then begin
        ...
        while histjson.Count > 5 * 3600/10 do
          histjson.Delete(0);
...
//      begin
//        jsonstr := jsonstr + memo2.Lines[i];
//      end;
//      str1 :=system.copy(memo2.Lines[memo2.Lines.Count - 1], 1, length(jsonstr) - 1);
//      jsonstr := jsonstr + str1;
