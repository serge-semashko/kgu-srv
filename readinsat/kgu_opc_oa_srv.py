...
def read_gamma100():
    global gamma_data, gamma_init
    url_gamma = "opc.tcp://10.0.10.60:54000/server/"
    while not gamma_init:
        try:
            client = Client(url_gamma)
            print("connect")
            client.connect()
            print("connected")
            gamma_init = True 
...
all_data = { "k1td1" :'NaN', "k1td2" : 'NaN', "k1td3" : 'NaN', "k1pg" : 'NaN', "k1sbr" : 0,
            ...
            'Gamma100_1_ch1': "-0,10453", 'Gamma100_2_ch1': "0", 'Gamma100_3_ch1': '0', 'Vlagomer_ch1_par2': '68,115234375'}
...
def run_flask():
    ...
    def get_gammadata():
        global lastInsatData
        test1=(str(gamma_data))
        outData = test1.replace('\'','\"')
        print('outdata = ' + outData)
                           
        resp = make_response(outData,200)
        resp.headers['content-type']='text/html'
        resp.headers['Access-Control-Allow-Origin']= '*'
        resp.headers['Access-Control-Allow-Methods'] ='GET, POST, PUT, DELETE, OPTIONS'
        return resp    
    ...
    def get_alldata():
        global lastInsatData, gamma_data
        test1=str({**lastInsatData,**gamma_data})
        outData = test1.replace('\'','\"')
        # print('outdata = ' + outData)
...
def getHTTP():
    ...
    while (True):
        ...
        url = "http://10.0.10.103:9090/get_current&callback=?";
        ...
        print('updsate opc ua')
        for i in indict:
            if i.find('time')>-1 or i.find('date') >-1:
                continue
...
get_websrv.start()
get_gamma100  = threading.Thread(target=read_gamma100, args=())
get_gamma100.start()
